    ...
    public partial class so : System.Web.UI.Page
    {
        ...
        protected void btnModalEnviarSOATM_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    string vQuery = "STEISP_ATMAdminComponentesATM 18, '" + Session["codsoATM"] + "','" + txtModalNewSOATM.Text + "', '" + Session["usuATM"].ToString() + "'";
        ...
        protected void btnModalNueviSOATM_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    string vQuery = "STEISP_ATMAdminComponentesATM 17, '" + Session["codsoATM"] + "','" + txtNewSOATM.Text + "','" + Session["usuATM"].ToString() + "'";
                    Int32 vInfo = vConexion.ejecutarSQL(vQuery);
                    if (vInfo == 1)
                    {
                        H5Alerta2.Visible = false;
