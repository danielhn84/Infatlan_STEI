    ...
    public partial class rechazados : System.Web.UI.Page
    {
        ...
        protected void GVBusqueda_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                ...
                GVBusqueda.DataSource = (DataTable)Session["ATM_RECHAZADOVERIF_CARGAR"];
                GVBusqueda.DataBind();
            }
            catch (Exception Ex)
            {
        ...
        {
            DataTable vDataaaa = (DataTable)Session["ATM_RECHAZADOVERIF_CARGAR"];
            string codVerificacion = e.CommandArgument.ToString();
            Session["ATM_RECHAZADO_VER"] = "1";
            if (e.CommandName == "Aprobar")
            {
                try
                {
                    DataTable vDatos = new DataTable();
                    String vQuery = "STEISP_ATM_Generales 26,'" + codVerificacion + "'";
